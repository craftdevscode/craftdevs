...
require_once __DIR__.'/core/tgm.php';
...
function gt3_set_options(){
	...
	if (class_exists('\Elementor\Plugin')) {
		...
		$features    = $experiments->get_active_features();
		foreach($features as $current_feature) {
			update_option($experiments->get_feature_option_key($current_feature['name']), $experiments::STATE_INACTIVE);
		}
	}
...
if(!function_exists('gt3_wp_body_classes')) {
	function gt3_wp_body_classes($classes){
		...
		if(gt3_option('sticky_sidebar')) {
			$classes[] = 'gt3_enable_sticky_sidebar';
		}
...
if(!function_exists('gt3_page_title')) {
	function gt3_page_title(){
		...
		if(class_exists('WooCommerce') && is_product()) {
			...
			$title = single_term_title("", false);
		} else if(is_archive() && get_queried_object() instanceof \WP_Term && in_array(get_queried_object()->taxonomy, array('portfolio_category', 'portfolio_tag'))) {
			...
			$title = esc_html__(sprintf('Results for: %s', get_search_query()), 'ewebot');
		} else if(is_404()) {
...
function gt3_the_breadcrumb(){
	...
	if(is_front_page() && !is_home()) {
		...
		if(is_category()) {
			...
			echo wp_kses_post($before).esc_html__('Category', 'ewebot').' "'.single_cat_title('', false).'"'.wp_kses_post($after);

		} else if(is_archive() && get_queried_object() instanceof \WP_Term && in_array(get_queried_object()->taxonomy, array('portfolio_category', 'portfolio_tag'))) {

			$term = get_queried_object();
			$title = esc_html__('Category', 'ewebot');
			...
			switch($term->taxonomy) {
				case 'portfolio_category':
					break;
				case 'portfolio_tag':
					$title = esc_html__('Tag', 'ewebot');
					break;
			}

			echo wp_kses_post($before).esc_html($post_type->labels->singular_name) . ' '.$title.' ' . '"' .  $term->name . '"' . wp_kses_post($after);
			...
			echo wp_kses_post($before).esc_html__('Search results', 'ewebot').wp_kses_post($after);
		...
		} else if(!is_single() && !is_page() && get_post_type() != 'post' && !is_404() /*&& !is_archive()*/) {
		...
		if (is_tag() || is_category()) {
			...
			if (!empty($tax->description)) {
				echo '<p class="gt3-taxonomy-description">' . $tax->description . '</p>';
			}
		}
...
if(!function_exists('gt3_preloader')) {
	function gt3_preloader(){
		...
		$preloader_mobile_disable = gt3_option('preloader_mobile_disable');
		if($post_loader && $mb_preloader == 'custom') {
			$preloader_mobile_disable = rwmb_meta('mb_preloader_mobile_disable', array(), $id);
		}

		$is_mobile = false;
		if ($preloader_mobile_disable == '1' || true === $preloader_mobile_disable) {
			$is_mobile = wp_is_mobile();
		}

		if((gt3_option('preloader') == '1' || gt3_option('preloader') === true || $mb_preloader == 'custom') && !$is_mobile) {
...
add_filter('gt3/core/docs/url', function() {
	return 'https://help.ewebotwp.com/docs/';
});


$has_shop_on_page = false;

add_filter( 'elementor/frontend/builder_content_data', function($data, $post_id) use (&$has_shop_on_page){
	if (!$has_shop_on_page) {
		$has_shop_on_page = find_shop_on_page($post_id);
	}
...
function gt3_has_shop_on_page() {
	...
	if(is_singular()) {
		...
		$is_shop  = find_shop_on_page($post->ID);
	}
...
function find_shop_on_page($post_id, $data = array()) {
	$is_shop = false;
	$meta = get_post_meta($post_id, '_elementor_controls_usage', true);
	$meta = maybe_unserialize($meta);

	if(is_array($meta) && (key_exists('gt3-core-shoplist', $meta) || key_exists('wpda-builder-cart', $meta))) {
		$is_shop = true;
	}
	if (!$is_shop) {
		if (class_exists('\Elementor\Plugin')) {
...
function gt3_find_elementor_widget(&$key){
	if(key_exists('widgetType', $key) && in_array($key['widgetType'], array( 'gt3-core-shoplist', 'wpda-builder-cart'))) {
		return true;
	}
